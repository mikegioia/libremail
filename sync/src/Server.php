...
class Server implements MessageComponentInterface
{
    ...
    {
        $this->log->debug( "New connection opened from #". $conn->resourceId );
        $this->clients->attach( $conn );
    }

    public function onClose ( ConnectionInterface $conn )
    {
        $this->log->debug( "Closing connection to #". $conn->resourceId );
    ...
    {
        $this->log->notice(
            "Error encountered from connection: ". $e->getMessage() );
        $conn->close();
    }

    public function onMessage ( ConnectionInterface $from, $message )
    {
        $this->log->debug( "New message from #{$from->resourceId}: $message" );
